...
このファイルは、Antigravity や Claude Code などの AI Agent が本リポジトリ (`m-pass`) で作業する際のプロジェクトガイドラインおよび指示を定義します。
...
1. **メンバー登録**: フォーム入力（アンケート含む） → API → 即時本登録・セッション発行 → マイページ遷移 ＆ 非同期で確認メール送信 → メール確認完了（リンクタップで検証・セッション発行 → マイページ表示）
...
- `POST /api/members/verify` - メール確認リンクの検証・メールアドレス検証完了処理（ログインセッション発行含む）
...
- `/register/verify` - メール確認処理（検証成功時にセッション発行しマイページへ自動遷移）
