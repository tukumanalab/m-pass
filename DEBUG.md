...
ステップ5: ブラウザで http://localhost:8080/member-debug/register
  → メンバー登録を実行
  → VSCodeでブレークポイントで停止！
```
...
```
http://localhost:8080/member-debug
http://localhost:8080/member-debug/register
```
...
| 本番環境     | Nginx 経由   | http://localhost:8080/member       |
| デバッグ環境 | 直接         | http://localhost:3001              |
| デバッグ環境 | Nginx 経由   | http://localhost:8080/member-debug |
...
# または
curl http://localhost:8080/member-debug/api/members

# 方法2: Node.jsスクリプトでテーブル作成を確認
docker exec m-pass-debug node -e "const db = require('better-sqlite3')('/app/data-debug/checkin-member-debug.db'); console.log('Tables:', db.prepare(\"SELECT name FROM sqlite_master WHERE type='table'\").all()); db.close();"
...
```bash
# データベースファイルを削除
rm -f checkin.db checkin-member.db checkin-member-debug.db
...
**方法 1: Node.js スクリプトで確認（推奨）**

```bash
# メンバー数を確認
docker exec m-pass-debug node -e "const db = require('better-sqlite3')('/app/data-debug/checkin-member-debug.db'); console.log('Members:', db.prepare('SELECT COUNT(*) as count FROM members').get()); db.close();"

# テーブル一覧を確認
docker exec m-pass-debug node -e "const db = require('better-sqlite3')('/app/data-debug/checkin-member-debug.db'); console.log('Tables:', db.prepare(\"SELECT name FROM sqlite_master WHERE type='table'\").all()); db.close();"

# 全メンバーを表示
docker exec m-pass-debug node -e "const db = require('better-sqlite3')('/app/data-debug/checkin-member-debug.db'); console.log('Members:', JSON.stringify(db.prepare('SELECT * FROM members LIMIT 5').all(), null, 2)); db.close();"
```
...
```bash
# データベースをホストにコピー
docker cp m-pass-debug:/app/data-debug/checkin-member-debug.db ./temp-db.db
...
# SQLiteコマンドで確認
docker exec m-pass-debug sh -c "echo 'SELECT COUNT(*) FROM members;' | sqlite3 /app/data-debug/checkin-member-debug.db"

# テーブル一覧を表示
docker exec m-pass-debug sh -c "echo '.tables' | sqlite3 /app/data-debug/checkin-member-debug.db"

# スキーマを表示
docker exec m-pass-debug sh -c "echo '.schema members' | sqlite3 /app/data-debug/checkin-member-debug.db"
...
```bash
# 1. データベースをホストにコピー
docker cp m-pass-debug:/app/data-debug/checkin-member-debug.db ./checkin-member-debug.db

# 2. VSCodeで「SQLite Viewer」拡張機能をインストール
# 3. VSCodeでcheckin-member-debug.dbファイルを開く
# 4. GUIでテーブルやデータを確認
```
...
| basePath 動作確認  | Docker + Nginx | `/member-debug` でアクセス可能   |
