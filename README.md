# M-Pass - QR コード入館管理システム

QR コードを使った利用者の入館管理 Web アプリケーション
...
- **QR コードスキャン**: カメラで QR コードをスキャンしてチェックイン
...
- 本番環境: http://localhost:8080/member
- デバッグ環境: http://localhost:8080/member-debug
...
- 本番環境（`/member`）とデバッグ環境（`/member-debug`）が同時に稼働
- デバッグ環境は別ポート（3001）で動作
- デバッグ環境は別データベース（`checkin-debug.db`）を使用
- 本番環境とデバッグ環境は完全に独立

アクセス URL：

- 本番環境: `https://tukumana.si.aoyama.ac.jp/member`
- デバッグ環境: `https://tukumana.si.aoyama.ac.jp/member-debug`
...
# ホストのDBをデバッグ環境にコピー
docker cp ./backup/checkin-member-debug-20251026.db m-pass-debug:/app/data-debug/checkin-member-debug.db
...
# - ブラウザで http://localhost:8080/member にアクセス
# - または curl http://localhost:8080/member

# 3. データベースが作成されたか確認
docker exec m-pass-prod ls -la /app/data/
# checkin-member.db ファイルが作成されている
```
...
const db = require('better-sqlite3')('/app/data/checkin-member.db');
